...
class CandyEngine:
                ...
                if c < self.cols - 1:
                    ...
                    if right_candy not in ['empty', 'blocker', 'unknown'] and current_candy != right_candy:
                        ...
                        if score > 0:
                            valid_moves.append({
                                'pos1': (r, c), 'pos2': (r, c + 1),
                                'direction': 'Right ➡️', 'score': score, 'details': match_details
                            })
                ...
                if r < self.rows - 1:
                    ...
                    if down_candy not in ['empty', 'blocker', 'unknown'] and current_candy != down_candy:
                        ...
                        if score > 0:
                            valid_moves.append({
                                'pos1': (r, c), 'pos2': (r + 1, c),
                                'direction': 'Down ⬇️', 'score': score, 'details': match_details
                            })
    ...
    def _evaluate_swap(self, pos1: Tuple[int, int], pos2: Tuple[int, int]) -> Tuple[int, str]:
        ...
        for m in [matches1, matches2]:
            if m['h_len'] >= 3:
                ...
                details.append(f"Horizontal {m['h_len']}")
            if m['v_len'] >= 3:
                total_score += self._calculate_score(m['v_len'])
                details.append(f"Vertical {m['v_len']}")
    ...
    def _check_matches_at(self, grid: np.ndarray, r: int, c: int) -> Dict:
        ...
        left = c
        while left > 0 and grid[r, left - 1] == color: left -= 1
        right = c
        while right < self.cols - 1 and grid[r, right + 1] == color: right += 1
        h_len = right - left + 1

        up = r
        while up > 0 and grid[up - 1, c] == color: up -= 1
        down = r
        while down < self.rows - 1 and grid[down + 1, c] == color: down += 1
        v_len = down - up + 1

        return {'h_len': h_len, 'v_len': v_len}

    def _calculate_score(self, match_length: int) -> int:
        if match_length == 3: return 10
        elif match_length == 4: return 30
        elif match_length >= 5: return 100
        return 0
